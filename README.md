# Coding Repository

This repository is a personal coding workspace containing practice projects in **Python**, **C**, and a small **portfolio web** section.

## Repository Structure

- `python/` — Multiple Python learning and project folders (games, mini apps, ML projects)
- `c/` — C language practice files
- `portfolios/` — Portfolio HTML and assets

## Notable Python Projects

### 1) Music Recommendation System
Path: `python/music recommendation project`

- Streamlit-based content recommendation app
- Includes its own README and requirements

Run:

```bash
cd "python/music recommendation project"
pip install -r requirements.txt
streamlit run app.py
```

### 2) Energy Simulator App
Path: `python/energy_simulator_app`

Run example script:

```bash
cd "python/energy_simulator_app"
python run_simulator.py
```

### 3) Python Game Projects
Examples:
- `snake game`
- `ping pong game`
- `turtle crossing`
- `states game`
- `quiz project`

Most can be run by opening the folder and running `main.py`.

## C Practice
Path: `c`

Contains practice files like:
- `chapter1.c`
- `chapter2.c`

Compile and run (example):

```bash
cd c
gcc chapter1.c -o chapter1
./chapter1
```

## Portfolio Files
Path: `portfolios`

Contains static HTML pages and images for portfolio presentation.

## Notes

- This repo is a mixed learning/project collection.
- Different folders have different dependencies.
- For project-specific details, check README files inside individual folders (where available).
